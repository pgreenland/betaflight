...
extern uint32_t vtxTablePowerValues[10];
extern const char* vtxTablePowerLabels[10];
...
#define MODE_RXTX (0)
extern serialPortConfig_t *findSerialPortConfig(int x);
extern serialPort_t *openSerialPort(int id, int func, void* x, void* y, int speed, int mode, portOptions_e opts);
extern void serialWriteBuf(serialPort_t* port, uint8_t* buf, int size);
extern int serialRxBytesWaiting(serialPort_t* port);
extern uint8_t serialRead(serialPort_t* port);
extern void vtxInit();
#define FUNCTION_VTX_TRAMP (0)
#define BOXVTXCONTROLDISABLE (0)
#define IS_RC_MODE_ACTIVE(x) (0)
...
static void vtxTrampProcess(vtxDevice_t *vtxDevice, timeUs_t currentTimeUs)
{
    ...
    switch(trampStatus)
    {
        ...
        case TRAMP_STATUS_ONLINE_MONITOR_FREQPWRPIT:
        {
            ...
            if (    (trampRetryCount > 0)
                 && (cmp32(currentTimeUs, trampLastTimeUs) >= TRAMP_MIN_REQUEST_PERIOD_US)) {
